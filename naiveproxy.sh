...
getDownload(){
	echo -e "https://raw.githubusercontent.com/small7nogp/proxyscripts/main/bin/naiveproxy/caddy-linux-$(archAffix)"
}
...
:$proxyport, $domain:$proxyport {
  ...
  reverse_proxy https://$proxysite {
    ...
    header_up X-Forwarded_Host {host}
  }
}
EOF
)
